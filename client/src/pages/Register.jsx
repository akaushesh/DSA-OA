...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const user = await authService.signup(form.fullName, form.username, form.password);
        ...
        <form onSubmit={handleSubmit} className="bg-[#1a1a2e] border border-[#2d2d44] rounded-xl p-8 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={form.fullName} onChange={e => setForm(f => ({...f, fullName: e.target.value}))}
              className="mt-1 w-full bg-[#0f0f1c] border border-[#2d2d44] text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500" required />
          </div>
          <div>
